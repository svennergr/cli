...
async function loadConfigurationFile(
  ...
  if (!(await fileExists(filepath)) && !allowEmpty) {
...
export async function parseConfigurationFile<TSchema extends zod.ZodType>(
  ...
  const configurationObject = await loadConfigurationFile(filepath, abortOrReport, decode, allowEmpty)
...
class AppLoader {
  ...
  parseConfigurationFile<TSchema extends zod.ZodType>(
    ...
    return parseConfigurationFile(schema, filepath, this.abortOrReport.bind(this), allowEmpty, decode)
  }
