...
import {AppConfiguration, AppInterface, isCurrentAppSchema, appIsLaunchable, getAppScopes} from '../models/app/app.js'
...
export async function fetchOrCreateOrganizationApp(
  ...
  const scopes = isCurrentAppSchema(app.configuration) ? getAppScopes(app.configuration) : app.configuration?.scopes
